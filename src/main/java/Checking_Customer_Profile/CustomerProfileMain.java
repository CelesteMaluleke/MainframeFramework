...
public class CustomerProfileMain {
    ...
    public void mainDebitOrder() throws JagacyException, InterruptedException, IOException
    {
        ...
        try {
            ...
            od_ = excel.columnsNames.indexOf("ODExists");
                    ...
                    if (runStatus.equalsIgnoreCase("RUN")) {
                        ...
                        if (userLoggedIn) {
                            ...
                            if (clientCode.equals("")) {
                                ...
                                if (clientProfile.equalsIgnoreCase("") || clientProfile.equalsIgnoreCase("NO CLIENT RECORD FOUND ENTER TO PROCEED TO NAME SEARCH")) {
                                    test.log(LogStatus.INFO, message);
                                    test.log(LogStatus.FAIL, "Client Profile  ");
                                    excel.WriteToCell("RUN", "Client Code Not Found", "Not Found",id_Number ,"Linked Account Not Found", "FAIL", "Not Checked", y, RunStatus_, Client_code_, aul_registered_, linked_Accounts_, results_, od_, id_Number_);
                                    ...
                                    checkOD = debitOrderJagacy.checkOD("CHQO SOD", accountNo, "4", "200", "2");

                                    if (checkOD.equalsIgnoreCase("INCREASE NOT ALLOWED IF NO OVERDRAFT EXISTS")) {
                                        checkOD = "Over-Draft do not exists..";
                                    }
                                    ...
                                    excel.WriteToCell("NO RUN", clientProfile, message, linkedAccounts.toString(),id_Number, "PASS", checkOD, y, RunStatus_, Client_code_, aul_registered_, linked_Accounts_, results_, od_, id_Number_);
                                    capture = sikuliScreen.saveScreenCapture(ReportFolder.screenshortReportPath, "Screen");
                                    String screenshotName = capture.split("\\\\")[capture.split("\\\\").length - 1];
                                    test.log(LogStatus.INFO, test.addScreenCapture(ReportFolder.screenshortReportPath + File.separator + screenshotName));
                                    debitOrderJagacy.close();
                                ...
                                test.log(LogStatus.INFO, "Client Profile ");
                                test.log(LogStatus.PASS, "Client Profile ");
                                excel.WriteToCell("NO RUN", clientCode, message, linkedAccounts.toString(),id_Number ,"PASS", "Not Checked", y, RunStatus_, Client_code_, aul_registered_, linked_Accounts_, results_, od_, id_Number_);
                                capture = sikuliScreen.saveScreenCapture(ReportFolder.screenshortReportPath, "Screen");
                                String screenshotName = capture.split("\\\\")[capture.split("\\\\").length - 1];
                                test.log(LogStatus.INFO, test.addScreenCapture(ReportFolder.screenshortReportPath + File.separator + screenshotName));
                                debitOrderJagacy.close();
                            ...
                            excel.WriteToCell("NO RUN", clientProfile, message, "Check Your User Credintials...", "FAIL", "Not Checked",id_Number, y, RunStatus_, Client_code_, aul_registered_, linked_Accounts_, results_, od_, id_Number_);
                            debitOrderJagacy.close();
                        }
                    }
                }catch (Exception e){
