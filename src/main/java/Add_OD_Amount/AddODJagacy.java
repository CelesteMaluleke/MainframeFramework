...
public class AddODJagacy extends Session3270 {
    ...
    public String AddOD(String userData,String accountNo, String requiredAction, String overdraftType, String aggrementSize) throws JagacyException {
        ...
        if(message.equalsIgnoreCase("INCREASE NOT ALLOWED IF NO OVERDRAFT EXISTS") || message.equalsIgnoreCase("NOT AUTHORIZED TO PROCESS OVERDRAFT")  || message.equalsIgnoreCase("ACCOUNT NUMBER DOES NOT EXIST") || message.equalsIgnoreCase("NO RAC-SITE FOR ACCOUNT DOMICILE") || message.equalsIgnoreCase("TRANSACTION NOT ALLOWED - UNDER DEBT COUNSELLING")){
            ...
            userIdRow = 8;
            userIdColumn = 25;
            this.writePosition(userIdRow, userIdColumn, "01062020");
    ...
    public String checkCasa(String idNumber) throws JagacyException {

        this.waitForChange(3000);
        userIdRow = 2;
        userIdColumn = 1;
        this.writePosition(userIdRow, userIdColumn, "CASA");
        this.writeKey(Key.ENTER);
        ...
        userIdColumn = 1;
        message = this.readPosition(userIdRow, userIdColumn, 60).trim();
        if(message.contains("Could not be found on CASA system")){
            ...
            userIdRow = 6;
            userIdColumn = 7;
            message = this.readPosition(userIdRow, userIdColumn, 200).trim();

            userIdRow = 21;
            userIdColumn = 17;
            this.writePosition(userIdRow, userIdColumn, "Cancel");
            this.writeKey(Key.ENTER);
            this.waitForChange(1000);
